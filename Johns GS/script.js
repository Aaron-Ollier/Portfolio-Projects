...
downloadBtn.addEventListener("click", function () {
  ...
  const ws_data = [["Customer Name", "Amount", "Date"]];  
  // Loop through table rows and add data to worksheet
  const rows = table.rows;
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const rowData = [customerName, row.cells[0].textContent, row.cells[1].textContent];
...
// Get the createBtn element
const createBtn = document.getElementById("createBtn2");

// Get the customerNameModal element
const customerNameInput = document.getElementById("customerNameInput");
const customerName = customerNameInput.value;
// Add a click event listener to the createBtn element
createBtn.addEventListener("click", function() {
  // Set the display style of the customerNameModal to "block"
  customerNameModal.style.display = "block";
  if (customerNameModal) {
...
customerNameModal.addEventListener("submit", function (event) {
  event.preventDefault();
  const customerNameInput = document.getElementById("customerNameInput");
  const customerName = customerNameInput.value;
  expenseTableTitle.textContent = customerName;
